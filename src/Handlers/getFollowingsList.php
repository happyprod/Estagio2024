...
if ($option == 'open') {
    $followers_list = $controller->getFollowingsList($id);
    ...
    foreach ($followers_list as $row) {
        ...
        if ($google_image) {
            ...
            $html .= 'src="http://localhost/redes/public/users/' . $fl_id . '/' . $fl_picture . '"';
        }
    ...
    $followers_list = $controller->getFollowingsListSearch($id, $id_name_user_search);
